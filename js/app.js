...
async function loadSprint() {
  ...
  // Placeholder (si deseas, luego lo calculamos desde SQL)
...
async function loadBurndownChartAndTable() {
  ...
  const completadas = data.map((r, i) =>
    (i === 0) ? 0 : Math.max(0, (real[i - 1] ?? real[i]) - (real[i] ?? real[i - 1]))
  );
  ...
  // Inputs: cambio de "Real" → set manual
  ...
  // Recalcular todo el burndown desde las cargas
...
} // <= cierre correcto de loadBurndownChartAndTable()
...
async function loadPorHistoria() {
  ...
  (data || []).forEach(h => {
    ...
    const pill = pct >= 0.8
      ...
      : (pct >= 0.5 ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-blue-50 text-blue-700 border-blue-200');
...
async function loadPorPropietario() {
  ...
  (data || []).forEach(o => {
    ...
    card.innerHTML = `<div class="font-semibold mb-2">${o.propietario}</div>
...
async function loadControlSub() {
  ...
  async function fetchPage() {
    ...
    if (qstr) {
      ...
      pageLbl.textContent = `${page} / ${Math.max(1, Math.ceil(total / limit))}`;
...
$('f_save').addEventListener('click', async () => {
  ...
  await supabase.rpc('reconstruir_burndown_sprint_activo'); // inicializa burndown del nuevo sprint
  await initDashboard();
});
...
async function importSub(file) {
  ...
  for (const r of rows) {
    ...
    const { error } = await supabase.rpc('upsert_subtarea', {
      p_id_subtarea, p_id_tarea_principal, p_titulo, p_estado, p_propietario, p_duracion_txt, p_fecha_terminacion
    });
...
$('u_process').addEventListener('click', async () => {
  ...
  try {
    ...
    await supabase.rpc('reconstruir_burndown_sprint_activo'); // una sola vez al final
